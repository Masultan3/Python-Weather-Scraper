...
weather_times = [item.find(class_="time j-time with-zeros").get_text() for item in box] 
weather_status = []
for item in box:
...
weather_temperatures = [i['data-temp'] for i in today.find_all('div', {'class':'temperature j-temp'})] 
weather_directions = [item.find(class_="direction-badge").get_text() for item in box]
weather_kt = [i['data-wind-speed'] for i in today.find_all('div', {'class':'wind-speed j-wind-speed'})]
